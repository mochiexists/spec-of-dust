...
2. Shell out to the other model's CLI to get feedback:
   - If you are Claude Code: `cat .spec/changes/{name}.md | codex -q "Review this spec using nearby repo context only when needed: the active change file, directly referenced files, and obviously adjacent files only. What's missing, over-engineered, ambiguous, risky, or broadly concerning? Prioritize concrete issues with this change. Treat broader critiques as advisory unless they reveal a real blocker: a missed requirement, bug, unsafe design, or contradiction with the spec. Be blunt. Reply in under 200 words."`
   - If you are Codex: `cat .spec/changes/{name}.md | claude -p "Review this spec using nearby repo context only when needed: the active change file, directly referenced files, and obviously adjacent files only. What's missing, over-engineered, ambiguous, risky, or broadly concerning? Prioritize concrete issues with this change. Treat broader critiques as advisory unless they reveal a real blocker: a missed requirement, bug, unsafe design, or contradiction with the spec. Be blunt. Reply in under 200 words."`
...
2. Shell out to the other model:
   - If you are Claude Code: `git diff --staged | codex -q "Review this diff against the spec in .spec/changes/{name}.md using nearby repo context only when needed: the spec, changed files, directly referenced files, and obviously adjacent files only. What bugs, missed requirements, risky assumptions, related errors, or style issues do you see? Prioritize concrete issues with this change. Treat broader critiques as advisory unless they reveal a real blocker: a missed requirement, bug, unsafe design, or contradiction with the spec. Be blunt. Under 300 words."`
   - If you are Codex: `git diff --staged | claude -p "Review this diff against the spec in .spec/changes/{name}.md using nearby repo context only when needed: the spec, changed files, directly referenced files, and obviously adjacent files only. What bugs, missed requirements, risky assumptions, related errors, or style issues do you see? Prioritize concrete issues with this change. Treat broader critiques as advisory unless they reveal a real blocker: a missed requirement, bug, unsafe design, or contradiction with the spec. Be blunt. Under 300 words."`
